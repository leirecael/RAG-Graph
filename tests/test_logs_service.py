...
from app.logic.logs_service import get_logs, get_log_statistics_by_type
...
def test_get_log_statistics_by_type_with_valid_data():
    ...
    with patch("app.logic.logs_service.read_data_logs", return_value=entries), \
         patch("app.logic.logs_service.read_error_logs", return_value=[]):
...
def test_get_logs_returns_correct_structure():
    ...
    with patch("app.logic.logs_service.read_data_logs", return_value=data_entries), \
         patch("app.logic.logs_service.read_error_logs", return_value=error_entries):
...
def test_log_statistics_ignores_non_numeric_costs_and_durations():
    ...
    assert round(emb_stats["avg_duration_s"], 1) == 1.0  # One is 2.0, one is coerced to 0
